	...
	if (type == TYPE_PLUS) {
		...
		if (copy) {
			...
			if (start) {
				...
				// NOTE: added to separate multi-part response
...
// This collects and returns things like APN, username, IP, from the user profile
// (UPSD command).
String CellularHelperClass::getPSDProfile(CellularPSDProfileParamTag paramTag) const {
	CellularHelperPlusStringResponse resp;
	// resp.enableDebug = true;
	resp.command = "UPSD";
	resp.delim = "|";

	String commandText = "UPSD=0";
	if (paramTag != CELL_PSD_PARAM_TAG_ALL) {
		commandText += "," + String(paramTag);
	}

	resp.resp = Cellular.command(responseCallback, (void *)&resp, DEFAULT_TIMEOUT, "AT+%s\r\n", commandText.c_str());

  if (resp.resp == RESP_OK) {
    return resp.string;
  }

	// empty string implies failure
	return "";
}
