...
int8_t Modbus::poll( uint16_t *regs, uint8_t u8size ) {
  ...
  // check slave id
  if (au8Buffer[ ID_ ] != u8id) return 0;
 ...
 * au8Buffer[] is the array containing the response from a slave device.
 * au8Buffer[2] indicates the number of bytes that holds the statuses of individual descrete inputs.
 * 1-byte contains status of 8 discrete inputs.
 * NB: Minimum of 9 discrete inputs must be read at once to get an output. Even if less than 9 inputs are desired, 9 inputs
 * should be read, then extract the statuses of interest.  
 * 
 * @ingroup register
...
void Modbus::get_FC1() 
{
  uint8_t buff_indx, i, byte_itr, array_indx;
